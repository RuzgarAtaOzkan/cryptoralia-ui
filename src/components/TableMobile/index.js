...
function TableMobile({ data, title = "Today's Best", desc = '', promotedTokens, votes = '' }) {
                ...
                return (
                  <Anchor key={index} className={cn(styles['promoted-row'], 'flx-btw-ctr')} href={'/' + token.name.replace(/\s/g, '-')}>
                    ...
                    <div className={cn(styles['logo'], 'flx-end-ctr')}>
                      <img title={token.displayName + ' Logo'} alt={token.name.replace(/\s/g, '-') + '-logo'} src={token.imgURL} width="40" height="40" loading="lazy" decoding="async" />
                    </div>
                    <div className={styles['symbol-name']}>
              ...
              {tokens.map((current, index) => {
                ...
                return (
                  <Anchor key={index} className={cn('flx-btw-ctr')} href={'/' + token.name.replace(/\s/g, '-')}>
                    ...
                    <div className={cn(styles['logo'], 'flx-end-ctr')}>
                      <img title={token.displayName + ' Logo'} alt={token.name.replace(/\s/g, '-') + '-logo'} src={token.imgURL} className={cn(styles['logo-img'])} width="40" height="40" loading="lazy" decoding="async" />
                    </div>
                    <div className={styles['symbol-name']}>
                      <div className={cn(styles['symbol'], 'flx-str-ctr')}>
                        {shortenText(token.symbol, 6)}
                ...
                {tokens.map((current, index) => {
                  ...
                  return (
                    <Anchor key={index} className={cn('flx-btw-ctr')} href={'/' + token.name.replace(/\s/g, '-')}>
                      ...
                      <div className={styles['network']}>
                        {token.network === 'BSC' ? <img alt="binance-logo" title="Binance Logo" src="https://ik.imagekit.io/kxi5qqh7r/bsc_ZJtG1WEa-.png?ik-sdk-version=javascript-1.4.3&updatedAt=1657230994783" /> : null}
                        <div>{token.network}</div>
                      </div>
